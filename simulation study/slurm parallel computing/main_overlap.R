...
# Number of parallel tasks: This corresponds to #SBATCH --array=1-50
# --------------------------------
num_parallel=50
...
### functions we need
{
  ...
  generate_combinations <- function(J, d) {
    # Initialize the master list to store all combination lists
    H_list_ind <- list()
    # Calculate the length of J
    q <- length(J)
    # Loop through each value of k from 1 to d
    for (k in 1:d) {
      # Generate combinations of indices from 1 to q, taken k at a time
      comb_matrix <- combn(q, k)
      # Convert each column of comb_matrix into a list element and add it to H_list_ind
      H_list_ind <- c(H_list_ind, lapply(1:ncol(comb_matrix), function(i) comb_matrix[, i]))
    }
    # Return the master list containing all combination lists
    return(H_list_ind)
  }
...
##### overlapping group lasso
{
  ...
  group_hierarchy <- function(J, d, ind_start_H, ind_end_H) {
    ...
    # Initialize group_TF matrix
    group_TF <- matrix(FALSE, nrow = max(ind_end_H), ncol = length(H_list_ind)+1)
    # Set the first column to TRUE
    group_TF[, 1] <- TRUE
    # Iterate through combinations in H_list_ind
    for (i in seq_along(H_list_ind)) {
      for (j in seq_along(H_list_ind)) {
        # Check if H_list_ind[i] is a subset of H_list_ind[j]
        if (is_subset(H_list_ind[[i]], H_list_ind[[j]])) {
          # Update group_TF based on ind_start_H and ind_end_H
          group_TF[ind_start_H[j+1]:ind_end_H[j+1], i+1] <- TRUE
        }
      }
    }
    # Return the group_TF matrix
    return(group_TF)
  }
  ...
  proximal_new <- function(beta_u, group_H_index, weight, lambda=1, iter_max=50, epsilon=1e-14){
    
    group_index=group_H_index
    # Initialize the weighted lambda value by scaling the input weight by lambda
    weight = weight * lambda
    # Get the number of groups from the number of columns in group_index
    ngroup = ncol(group_index)
    # Initialize a 3D array to store group information, with dimensions matching beta_u and ngroup
    group = array(0, dim=c(dim(beta_u), ngroup))
    # Copy the initial beta_u values to beta for manipulation
    beta = beta_u
    # Initialize a vector to track the feasibility of solutions for each group
    feasible = rep(1, ngroup)
    # Iterate up to iter_max times to find a solution
    for(j in 1:iter_max){
      for(g in 1:ngroup){
        # Check the feasibility by calculating the difference after projection
        # This checks if the current solution is within the allowed threshold
        dif = group[group_index[,g],,g] - project_ball(beta[group_index[,g],] + group[group_index[,g],,g], weight[g])
        feasible[g] = sum(abs(dif))
        # Update beta values for the current group based on the projection
        beta[group_index[,g],] = beta[group_index[,g],] + group[group_index[,g],,g]
        # Project the updated beta values and store in the group array
        group[group_index[,g],,g] = project_ball(beta[group_index[,g],], weight[g])
        # Adjust beta values by subtracting the projected group values
        beta[group_index[,g],] = beta[group_index[,g],] - group[group_index[,g],,g]
      }
      # Break the loop if the solution is feasible for all groups or if the iteration limit is reached
      if(sum(feasible) < epsilon | j == iter_max){
        break
      }
    }
    # Return the final beta values after all iterations
    # return(list(beta=beta, feasible=feasible))
    return( beta )
  }
  
  proximal_new_feasible <- function(beta_u, group_index, weight, lambda=1, iter_max=1000, epsilon=1e-14){
    # Initialize the weighted lambda value by scaling the input weight by lambda
    weight = weight * lambda
    # Get the number of groups from the number of columns in group_index
    ngroup = ncol(group_index)
  ...
  overlapping_group_lasso_proximal <- function(beta_u, group_H_index, ind_start_X, ind_end_X, weight, lambda=1, iter_max=1000, epsilon=1e-14) {
    
    # Ensure weight is a vector of length ngroup
    for_max=length(ind_end_X)
    # Initialize the final matrix
    final <- matrix(0, nrow = nrow(beta_u), ncol = ncol(beta_u))
    for (for_ind in 1:for_max ) {
      final[, ind_start_X[for_ind]:ind_end_X[for_ind] ] =  proximal_new(beta_u=as.matrix(beta_u[, ind_start_X[for_ind]:ind_end_X[for_ind] ]), group_H_index=group_H_index, weight=weight[,for_ind], lambda=lambda, iter_max=50, epsilon=1e-8) 
    }
    return(final)
  }
  
  
  calculate_l2_norms <- function(beta, group_H_index, ind_start_X, ind_end_X) {
    
    total_norm <- 0  # Initialize total norm
    beta2=beta^2
    # Loop through each group defined by columns in group_H_index
    for (i in seq_len(ncol(group_H_index))) {
      # Loop through each segment defined by ind_start_X and ind_end_X
      for (j in seq_along(ind_start_X)) {
        # Identify rows in beta that correspond to the current group (where group_H_index is TRUE)
        rows_in_group <- which(group_H_index[, i])
        # Extract the relevant segment of beta for the current group and range
        beta_segment <- beta2[rows_in_group, ind_start_X[j]:ind_end_X[j]]
        # Calculate the L2 norm for the current segment and add to the total
        total_norm <- total_norm + sqrt(sum(beta_segment))
      }
    }
    return(total_norm)
  }
  ...
  calculate_l2_norms_weight <- function(beta, group_H_index, ind_start_X, ind_end_X, w ) {
    ...
    for (i in seq_len(ncol(group_H_index))) {
      ...
      for (j in seq_along(ind_start_X)) {
        ...
        beta_segment <- beta2[rows_in_group, ind_start_X[j]:ind_end_X[j]]
        # Calculate the L2 norm for the current segment and add to the total
        total_norm <- total_norm + sqrt(sum(beta_segment))*w[i, j]
      }
    }
    return(total_norm)
  }
  ...
  backtracking_overlap_PGD_single <- function(gradient, loss, beta_init = NULL, H, X, Y, step_size, ind_start_H, ind_end_H, ind_start_X, ind_end_X, lambda, group_H_index, shrinking_gamma = 0.618, epsilon = 10^-8, max_iter = 1000, min_iter = 10, w = 1) {
    # Constants
    gamma = shrinking_gamma
    L_sum = ncol(H)
    eta = step_size
    ...
    grad = gradient(z, H, X, Y)
    repeat {
      beta_loop_new = overlapping_group_lasso_proximal( z - eta * grad, group_H_index, ind_start_X, ind_end_X, w = w, lambda = lambda * eta )
      # Calculate D_loop
      D_loop = beta_loop_new - z
      # Check step - Adjust eta if the condition is met
      log_loss_beta = loss(beta_loop_new, H, X, Y)
    ...
    ###
    cst_old = log_loss_beta + lambda * calculate_l2_norms_weight(beta_loop_new, group_H_index, ind_start_X, ind_end_X, w)  # Update for group lasso penalty
    for (i in 1:max_iter) {
      z = beta_loop_new + i / (i + 3) * (beta_loop_new - beta_loop)
      beta_loop = beta_loop_new
      grad = gradient(z, H, X, Y)
      ...
      cost[i] = log_loss_beta + lambda * calculate_l2_norms_weight(beta_loop_new, group_H_index, ind_start_X, ind_end_X, w)  # Update for group lasso penalty
      cst_new = cost[i]
      if (cst_old - cst_new < epsilon & sum(D_loop^2) < epsilon) {
        if (i > min_iter) {
          beta_final = beta_loop_new
  ...
  backtracking_overlap_PGD_path<-function(gradient,loss,H,X,Y,step_size,ind_start_H,ind_end_H,ind_start_X,ind_end_X,lam_seq,shrinking_gamma=0.618,epsilon=10^{-8},group_H_index,max_iter=100,min_iter=10,w=1){
    ...
    nlambda=length(lam_seq)
    #lambda_max
    ot=backtracking_overlap_PGD_single(gradient,loss,beta_init=NULL,H,X,Y,step_size,ind_start_H,ind_end_H,ind_start_X,ind_end_X,lam_seq[1],group_H_index,shrinking_gamma,epsilon,max_iter,min_iter=min_iter,w)  
    eta=ot$eta_end
    beta_tensor=array( rep(0, prod(dim(ot$beta_final))*nlambda ),dim=c(ncol(H),nrow(X) ,nlambda)   )
    beta_tensor[,,1]=ot$beta_final
    #  training_loss=matrix(rep(0, max_iter*nlambda ) , nrow=max_iter  )
    lam_ind=1
    for(lambda in lam_seq[-1]){
      ot=backtracking_overlap_PGD_single(gradient,loss,beta_init=beta_tensor[,,lam_ind],H,X,Y,step_size=eta/gamma,ind_start_H,ind_end_H,ind_start_X,ind_end_X,lambda,group_H_index,shrinking_gamma,epsilon,max_iter,min_iter=min_iter,w)  
      eta=ot$eta_end
      lam_ind=lam_ind+1
      beta_tensor[,,lam_ind]=ot$beta_final
    }
...
for(ind_loop in 1:nrow(params)  ){
  ...
  ### generate initial beta
  {
    ...
    {weight_deep_log_ratio=0
      weight_deep=exp(seq(from=0, by=weight_deep_log_ratio, length.out=length(J) ) )
      weight_groupX_x_w=1
      weight_groupX=c(1, rep(weight_groupX_x_w,length(ind_end_X)-1 ) )
      weight = weight_generate( J, d, ind_end_H, ind_end_X, weight_deep, weight_groupX,  scheme_X=2 ) }
  ...
  for( i in ID_index+(0:(nreps/num_parallel-1) )*num_parallel ){
    
    {
      ii=1
      nlam=80
      H_dis=matrix(0, nrow=1, ncol=2)
      mis_class=matrix(0, nrow=1, ncol=2)
      F_measure=matrix(0, nrow=1, ncol=2)
      G_measure=matrix(0, nrow=1, ncol=2)
      FPR_mat=matrix(0, nrow=1, ncol=2)
      FNR_mat=matrix(0, nrow=1, ncol=2)
      ...
      ind_end_X = 1:p
      ind_start_X = c(1, 1+ind_end_X[-length(ind_end_X)]) 
      group_H_index=group_hierarchy(J, d, ind_start_H, ind_end_H)
      ### 1. O-Mult
      lambda.min.ratio=0.90
      ...
      # training
      beta_path_O_mult=backtracking_overlap_PGD_path(grad_ll_mult,ll_mult,H,X=X_train,Y=Y_train,step_size=eta_mult,ind_start_H,ind_end_H,ind_start_X,ind_end_X,lam_seq_mult,shrinking_gamma=0.618,epsilon=epsilon,group_H_index,max_iter=100,min_iter=2,w=weight)
#beta_path_L_mult=backtracking_PGD_path(grad_ll_mult,ll_mult,H,X=X_train,Y=Y_train,step_size=eta_mult,ind_start_H,ind_end_H,ind_start_X,ind_end_X,lam_seq_mult,shrinking_gamma=0.618,epsilon=epsilon,max_iter=1000, w=weight)
      ...
      beta_tensor=beta_path_O_mult$beta_tensor
      # validation
      ...
      # training
      beta_path_O_pois=backtracking_overlap_PGD_path(grad_ll_pois,ll_pois,H,X=X_train,Y=Y_train,step_size=eta_pois,ind_start_H,ind_end_H,ind_start_X,ind_end_X,lam_seq_pois,shrinking_gamma=0.618,epsilon=epsilon,group_H_index,max_iter=100,min_iter=2,w=weight)
#beta_path_O_pois=backtracking_PGD_path(grad_ll_pois,ll_pois,H,X=X_train,Y=Y_train,step_size=eta_pois,ind_start_H,ind_end_H,ind_start_X,ind_end_X,lam_seq_pois,shrinking_gamma=0.618,epsilon=epsilon,max_iter=1000, w=weight)
      beta_tensor=beta_path_O_pois$beta_tensor
      # validation
    ...
    # -----------------------------------------------
    savename <- paste("/home/shenx/zhao1118/AD-simulation/structure_learning_2/data_overlap/Model", Model, "_p", p, "_n", n, "_repID", i, ".RDS", sep="")
